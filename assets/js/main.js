const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwqnkiSiwlZEir6MDjSTk43KNw_4mmuYqLLafsE7adArL36vjAIuRn5HaoSaPclTH0drw/exec';
...
function scrollToAnchor(href, behavior = 'smooth') {
...
async function handleFormSubmit(event, formType, successId) {
  ...
  data.client_submission_id = `${formType}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
// Mobile Bottom Nav Active State Tracking
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const activeItem = document.querySelector(`.bottom-nav-item[data-section="${entry.target.id}"]`);
...
// Hide bottom nav on scroll down, show on scroll up (mobile UX)
let lastScrollTop = 0;
